...
Source of truth: cmd/scaffold/internal/registry/registry.go

Idempotent behavior:
- Issues are identified by exact title:
    "[coverage] Implement <service>:<resource>"
- Only missing issues are created (existing ones are skipped).

This is intended to run in GitHub Actions using the built-in GITHUB_TOKEN.
Do NOT use or commit a personal access token (PAT).
"""
...
def parse_registry(registry_go: str) -> List[Resource]:
    """
    Small purpose-built parser for cmd/scaffold/internal/registry/registry.go.

    Extracts resources like:
    ...
    for raw_line in registry_go.splitlines():
        ...
        if in_resources and current_service:
            ...
            # end of resources map (this file uses "}," for many blocks)
            if line.strip() == "},":
                in_resources = False
                continue
...
This should create/update (depending on what exists already):
- `pkg/audit/{r.service}/{r.resource}.go` + `_test.go`
- `pkg/policy/validation/{r.service}.go`
- `e2e/{r.service}_test.go`
- `examples/policies/{r.service}-policy-guide.md`

### B) Implement discovery (real OpenStack listing)
Edit:
- `pkg/discovery/services/{r.service}.go`

Replace placeholder discovery with real OpenStack listing for `{r.resource}`:
- Get the correct service client (via `pkg/auth`)
- List resources with pagination
- Emit `discovery.Job` with real IDs + project + resource payload
...
def ensure_label(token: str, owner: str, repo: str, name: str, color: str, description: str, dry_run: bool) -> None:
    # If label exists, do nothing; else create.
    label_url = f"https://api.github.com/repos/{owner}/{repo}/labels/{name}"
    try:
        gh_request(token, "GET", label_url)
        return
    except RuntimeError:
        pass
...
def main() -> int:
    ...
    ap.add_argument("--dry-run", action="store_true", help="Print actions without creating/updating on GitHub")
    args = ap.parse_args()
    ...
    resources = parse_registry(reg_path.read_text(encoding="utf-8"))
