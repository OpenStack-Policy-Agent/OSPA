...
// InstanceAuditor audits nova resources of type instance
//
// TODO(OSPA): Replace placeholder logic with real field extraction + rule evaluation for nova/instance.
...
func (a *InstanceAuditor) Check(ctx context.Context, resource interface{}, rule *policy.Rule) (*audit.Result, error) {
	...
	// TODO(OSPA): Parse 'resource' into the correct OpenStack SDK type for nova/instance.
...
func (a *InstanceAuditor) Fix(ctx context.Context, client interface{}, resource interface{}, rule *policy.Rule) error {
	...
	case "log":
		return nil
	default:
		return fmt.Errorf("%s.%s fix action %q not implemented", "nova", "instance", rule.Action)
	}
}
