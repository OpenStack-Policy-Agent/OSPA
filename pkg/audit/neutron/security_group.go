...
// SecurityGroupAuditor audits neutron resources of type security_group
//
// TODO(OSPA): Replace placeholder logic with real field extraction + rule evaluation for neutron/security_group.
...
func (a *SecurityGroupAuditor) Check(ctx context.Context, resource interface{}, rule *policy.Rule) (*audit.Result, error) {
	...
	// TODO(OSPA): Parse 'resource' into the correct OpenStack SDK type for neutron/security_group.
...
func (a *SecurityGroupAuditor) Fix(ctx context.Context, client interface{}, resource interface{}, rule *policy.Rule) error {
	...
	case "log":
		return nil
	default:
		return fmt.Errorf("%s.%s fix action %q not implemented", "neutron", "security_group", rule.Action)
	}
}
